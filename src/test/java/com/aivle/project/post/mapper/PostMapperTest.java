...
class PostMapperTest {
	...
	void toResponseWithQnaStatus_shouldInjectGivenStatus() {
		...
		// when
		PostResponse pending = postMapper.toResponseWithQnaStatus(post, "pending");
		PostResponse answered = postMapper.toResponseWithQnaStatus(post, "answered");

		// then
		assertThat(pending.qnaStatus()).isEqualTo("pending");
		assertThat(answered.qnaStatus()).isEqualTo("answered");
	}
