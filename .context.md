...
* `feat(auth)` 비밀번호 복잡도 정책 적용 (SignupRequest)
* `feat(comment)` 댓글(Comments) 서비스 및 DTO 구현
* `feat(domain)` 댓글(Comments) 엔티티 및 리포지토리 추가 (Adjacency List Model)
* `docs(erd)` 댓글 도메인 SQL 스냅샷 추가 (`comments-domain.sql`)
