...
* `feat(auth)` 로그아웃/전체 로그아웃 및 Access Token 블랙리스트 추가
* `test(auth)` 로그아웃/블랙리스트 관련 단위 테스트 추가
* `fix(auth)` 전체 로그아웃 기준 시각 검증 보정 (동일 시각 허용)
* `feat(auth)` 회원가입 API 및 USER 역할 매핑 추가
* `test(auth)` RefreshTokenServiceTest 불필요 스텁 제거
...
* **2026-01-21**
    ...
    * 로그아웃/전체 로그아웃 및 Access Token 블랙리스트 구현
